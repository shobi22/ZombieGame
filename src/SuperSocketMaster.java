...
public class SuperSocketMaster implements Runnable {
	...
	// Special stucture that is used to hold the SuperSocketListener of each
	// connected clients.
	public Vector<SuperSocketListen> portconnections = new Vector<SuperSocketListen>();

	// Server Mode Constructor
	SuperSocketMaster(JTextField inTxtField, int intSckt) {
		blnServerMode = true;
		inTextField = inTxtField;
		strServerIP = "";
		intSocket = intSckt;
	}

	// Client Mode Constructor
	SuperSocketMaster(JTextField inTxtField, String strIP, int intSckt) {
		blnServerMode = false;
		inTextField = inTxtField;
		strServerIP = strIP;
		intSocket = intSckt;
	...
	public void run() {
		if (blnServerMode == true) {
			...
			try {
				...
				System.out.println("Success creating server. IP: " + strMyIP
						+ " Hostname: " + strMyHostname);
			} catch (IOException e) {
			...
			while (true) {
				...
				try {
					...
					outBuffer = new PrintWriter(socketObject.getOutputStream(),
							true);
					System.out.println("Server: got connection from "
							+ socketObject.toString() + "\n");
					String strClientIP = socketObject.getInetAddress()
							.getHostAddress();
					String strClientHostname = socketObject.getInetAddress()
							.getHostName();

					// Creating a thread to hande the inividial client
					// connection
					SuperSocketListen singleconnection = new SuperSocketListen(
							inTextField, outBuffer, inBuffer,
							socketObject.toString(), this, strClientIP,
							strClientHostname);
					portconnections.addElement(singleconnection);
					inTextField.setText("CONN - "
							+ singleconnection.strConnection);
					inTextField.postActionEvent();
					System.out.println("Elements in vector after add: "
			...
			try {
				...
				strMyIP = InetAddress.getLocalHost().getHostAddress();
				strMyHostname = InetAddress.getLocalHost().getHostName();
				System.out.println("Success creating client. IP: " + strMyIP
						+ " Hostname: " + strMyHostname);
				outBuffer = new PrintWriter(socketObject.getOutputStream(),
						true);
				inBuffer = new BufferedReader(new InputStreamReader(
	...
	public void removeClient(SuperSocketListen sslclient) {
		inTextField.setText("DISC - " + sslclient.strConnection);
		System.out.println("Removing Client" + sslclient.strConnection + ": "
				+ portconnections.remove(sslclient));
		System.out.println("Number of Clients after remove: "
				+ portconnections.size());
		inTextField.postActionEvent();
	}
	...
	public class SuperSocketListen implements Runnable {
		...
		public SuperSocketListen(JTextField incomingTxt, PrintWriter outgoing,
				BufferedReader incoming, String strConn, SuperSocketMaster ssm,
				String strClientIP, String strClientHostname) {
			inTxt = incomingTxt;
			out = outgoing;
			in = incoming;
		...
		public void run() {
			...
			while (strLine != null) {
				try {
					strLine = in.readLine();
					if (strLine != null) {
						System.out.println("Recieving Client " + strClientIP
								+ " Message: " + strLine);
						inTxt.setText(strLine);
						inTxt.postActionEvent();
					}
				} catch (IOException e) {
					// If the client disconnects, you will get an error.
					// Disconnect
					// BTW, the while loop runs while there is data. Thus the
					// while will end and the thread should end as well.
		...
		public void sendText(String strText) {
			...
			if (out.checkError()) {
				System.out.println("Client Write failed Closing Connection: "
						+ strConnection);
				in = null;
				out = null;
				strLine = null;
